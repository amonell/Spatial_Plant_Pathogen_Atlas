...
def display_fovs(merfish, remove_fovs=20, key='Cluster'):
    spatial_points = np.array([merfish.obsm['spatial'][:,0], merfish.obsm['spatial'][:,1]]).T
    ...
    plt.scatter(merfish.obsm['spatial'][:,0], merfish.obsm['spatial'][:,1], c=mfish, s=1, alpha=0.3)
    for j in range(len(binned_centers)):
        plt.text(np.array(binned_centers)[j, 0], np.array(binned_centers)[j, 1], j, fontsize=10, color = 'blue')
    plt.grid(False)
    plt.show()
...
def remove_fovs(merfish, fovs_to_remove, binned_centers, key='Cluster'):
    ...
    merfish =merfish[[i for i, e in enumerate(fov_associated) if e not in fovs_to_remove], :]
